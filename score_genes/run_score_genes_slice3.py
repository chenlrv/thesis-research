"""score_genes recipe on slice 3 (control) with combined FDR + margin calls.

normalize_total -> log1p -> sc.tl.score_genes per label (markers from
annotation_cell_marker_genes.MARKER_GENES). Each non-tumor cell is assigned only
if it passes BOTH a 5% mirrored-FDR gate and a MAD-scaled margin gate (top score
>= 1.5x the runner-up, the "50% rule"); else 'unknown'. Outputs scores + figures.
"""
import os
import re
import h5py
...
def main():
    ...
    for ax in axes[len(labels):]:
        ax.axis("off")
    fig.suptitle(
        "slice_3 per-cell score_genes distributions",
        fontsize=14,
    )
    ...
    for ax in axes[len(labels):]:
        ax.axis("off")
    fig.suptitle("slice_3 per-cell scores in space (red = high)", fontsize=15)
    ...
    for label in labels:
        ...
        ax.set_aspect("equal"); ax.set_xticks([]); ax.set_yticks([])
        ax.set_title(f"slice_3 {label} cells (n={int(mask.sum())}, {pct_all:.1f}% all cells)")
    ...
    print(f"\nsaved cell_scores.csv, mirrored_fdr_thresholds.csv, "
          ...
          f"and result_spatial_<celltype>.png files "
          f"-> {OUT_DIR}")
