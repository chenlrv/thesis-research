...
def get_negative_system_probes(adata: AnnData) -> AnnData:
    ...
    print(f'Found {probes.n_vars} negative/system-control probes in {sample_id}: {list(probes.var_names)}')
...
def remove_negative_system_probes(adata: AnnData, probes: AnnData) -> AnnData:
    ...
    if probes.n_vars > 0:
        ...
        print(f'Removed {len(probe_names)} probes from Anndata {sample_id}.\n'
