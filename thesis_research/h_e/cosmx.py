...
ax.scatter(
    adata.obs["CenterX_global_px"], adata.obs["CenterY_global_px"], s=0.1, c="black", alpha=0.1
)
...
def onclick(event):
    ...
    if event.button == 1:
        ...
        ax.annotate(str(len(cosmx_pts)), (event.xdata, event.ydata), color="red", fontsize=12)
