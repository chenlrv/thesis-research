...
def prepare_gnn_labels(adata_slice_1, adata_slice_3, df_results_slice_1):
    ...
    healthy_mask = (df_results_slice_1["predicted_cell_type"] != "Tumor") & (
        df_results_slice_1["next_best_score"] > 0.5
    )  # Using the healthy reference score
    high_conf_healthy_barcodes = set(df_results_slice_1.loc[healthy_mask, "cell_barcode"])

    # Apply to adata_slice_1
    adata_slice_1.obs["gnn_label"] = 0  # Default to 0
    ...
    conf_map = dict(zip(df_results_slice_1["cell_barcode"], df_results_slice_1["score_tumor"]))
    ...
    adata_slice_3.obs["singler_conf"] = 0.0
...
def create_advanced_graph(
    ...
    label_col: str = "singler_label",
    conf_col: str = "singler_conf",
):
    ...
    if conf_col not in adata_slice.obs.columns:
        raise KeyError(f"{conf_col} not found in adata_slice.obs")
    ...
    is_anchor = adata_slice.obs[conf_col].to_numpy() >= conf_threshold
    ...
    if is_slice_3:
        # You explicitly chose to use slice 3as healthy anchors
        y = torch.zeros(n, dtype=torch.long)
    else:
        y_np = adata_slice.obs[label_col].to_numpy()
        if not np.isin(y_np, [0, 1]).all():
...
# Training
# -----------------------------
def build_class_weights(data_a, data_b, device):
...
def train_model(model, data_a, data_b, optimizer, criterion, device, epochs=300, print_every=20):
...
def run_flow():
    ...
    # 5. Handle Imbalance
    class_weights = build_class_weights(data_a, data_b, device=device)
    criterion = torch.nn.NLLLoss(weight=class_weights)

    # 6. Train
    # Note: train_model moves data to device internally based on your code
    model, data_a_dev, data_b_dev = train_model(
        model=model,
        data_a=data_a,
        data_b=data_b,
        optimizer=optimizer,
