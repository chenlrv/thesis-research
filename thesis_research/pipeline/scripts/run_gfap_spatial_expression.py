"""Spatial plots of GFAP expression across all 6 slices.

Cells with GFAP >= 1 raw count are plotted and colored by expression level.
...
OUTPUT    = "D:/thesis-research/gfap_spatial_expression.png"
...
def draw_panel(ax, expr, tumor_mask, x, y, slice_id, vmax=None):
    ...
    gfap_mask  = (expr >= 1) & non_tumor
    bg_mask    = non_tumor & ~gfap_mask

    ax.scatter(x[bg_mask], y[bg_mask],
               c="#E0E0E0", s=0.4, alpha=0.15, linewidths=0, rasterized=True)

    sc = ax.scatter(x[gfap_mask], y[gfap_mask],
                    c=expr[gfap_mask], cmap="YlOrRd",
                    s=2.5, alpha=0.95, linewidths=0, rasterized=True,
                    vmin=1, vmax=vmax if vmax is not None else (expr[gfap_mask].max() if gfap_mask.any() else 1))
    ...
    ax.set_aspect("equal")
    ax.axis("off")
    ax.set_title(f"Slice {slice_id}  —  {gfap_mask.sum():,} / {non_tumor.sum():,}", fontsize=10)

    print(f"  Slice {slice_id}: {gfap_mask.sum():,} GFAP+ non-tumor cells  "
          f"(max expr = {expr[gfap_mask].max() if gfap_mask.any() else 0})")
...
print(f"Global GFAP max across all slices: {global_max}\n")
