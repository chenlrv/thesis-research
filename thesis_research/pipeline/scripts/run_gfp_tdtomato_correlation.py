"""Scatter plot: GFP vs tdTomato correlation — all 6 slices, two figures.
...
OUTPUT_1  = "D:/thesis-research/gfp_tdtomato_correlation_slices1-3.png"
OUTPUT_2  = "D:/thesis-research/gfp_tdtomato_correlation_slices4-6.png"
...
def load_slice(paths, slice_id):
    ...
    tdt = get_expr(adata, "tdTomato")[non_tumor]

    keep = (gfp >= 1) | (tdt >= 1)
    return gfp[keep], tdt[keep]


def draw_panel(ax, gfp, tdt, slice_id):
    r, _ = pearsonr(gfp, tdt)
    ...
    ax.scatter(gx, ty, s=counts / counts.max() * 1200,
               alpha=0.6, color="tomato", linewidths=0.4, edgecolors="white")
    ...
    for ref in [10, 100, 1000]:
        ax.scatter([], [], s=ref / counts.max() * 1200,
                   color="tomato", alpha=0.6, label=f"n={ref:,}")
...
def plot_group(slice_ids, output):
    ...
    fig.suptitle("GFP vs tdTomato (non-tumor cells expressing ≥ 1 count)", fontsize=13)
    for ax, slice_id in zip(axes, slice_ids):
        gfp, tdt = load_slice(SLICES[slice_id], slice_id)
        draw_panel(ax, gfp, tdt, slice_id)
    plt.tight_layout()
